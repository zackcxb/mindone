...
from examples.dit_infer_acceleration.DiTCache.pipeline_flux_withDeltaCache import FluxPipeline
import os
import argparse
import sys
import time
...
def arg_parse():
    ...
    parser.add_argument("--save_graph", action="store_true", help="Save IR graph for debugging.")
    ...
    parser.add_argument("--cache_config", type=str, default=os.path.join(parent_path, "config/cache_params.json"), help="Cache config")
...
def main():
    ...
    if args.load_ckpt:  
        pipe = FluxPipeline.from_pretrained(
            args.ckpt,
            custom_pipeline=os.path.join(parent_path, "pipeline_flux_withDeltaCache.py"),
        ...
        from transformers import CLIPTokenizer, T5TokenizerFast, CLIPImageProcessor
