...
def single_transformer_blocks_withcache(
    ...
    cache_start, cache_end = single_cache_idx
    save_start, save_end = single_save_idx
    ...
    if cache_start==cache_end:
        if save_start == save_end:
            ...
            single_delta_cache = hidden_states - hidden_states_before_cache
            for index_block, block in enumerate(blocks[save_end:]):
                hidden_states = block(
                    hidden_states=hidden_states,
                    temb=temb,
        ...
        hidden_states = hidden_states_before_cache + single_delta_cache
    ...
    return hidden_states, single_delta_cache
...
def transformer_blocks_withcache(
    ...
    return (encoder_hidden_states, hidden_states), delta_cache, delta_cache_encoder
...
def flux_transformer2d_construct(
    ...
    delta_cache_encoder: ms.Tensor = None,
    single_delta_cache: ms.Tensor = None,
        ...
        ((encoder_hidden_states, hidden_states), delta_cache, delta_cache_encoder) = self.transformer_blocks_with_cache(    
                hidden_states=hidden_states,
                encoder_hidden_states=encoder_hidden_states,
                temb=temb,
                image_rotary_emb=image_rotary_emb,
        ...
        (hidden_states, single_delta_cache) = self.single_transformer_blocks_with_cache(
                ...
                image_rotary_emb=image_rotary_emb,
                joint_attention_kwargs=joint_attention_kwargs,
                single_cache_idx=single_cache_idx,
                single_save_idx=single_save_idx,
        ...
        if not return_dict:
            return output, delta_cache, delta_cache_encoder, single_delta_cache
