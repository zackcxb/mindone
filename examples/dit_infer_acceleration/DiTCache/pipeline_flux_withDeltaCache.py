...
from mindone.diffusers.loaders import FluxIPAdapterMixin, FluxLoraLoaderMixin, FromSingleFileMixin, TextualInversionLoaderMixin
from mindone.diffusers.models import AutoencoderKL, FluxTransformer2DModel
...
from mindone.diffusers.pipelines.flux import FluxPipelineOutput
from examples.dit_infer_acceleration.DiTCache.models.transformers.flux_transformer_withDeltaCache import transformer_blocks_withcache, single_transformer_blocks_withcache, flux_transformer2d_construct
...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        ...   custom_pipeline="./", # path where this pipeline_flux_withDeltaCache.py in
...
class FluxPipeline(
    ...
    def __init__(
        ...
        cache_config: str = "./config/cache_params.json",
    ):
        super().__init__()
        ...
        transformer_skip = json.load(open(cache_config, "r"))["transformer_skip"]
        single_transformer_skip = json.load(open(cache_config, "r"))["single_transformer_skip"]
        transformer_skip = [tuple(l) for l in transformer_skip]
        single_transformer_skip = [tuple(l) for l in single_transformer_skip]
        self.transformer_skip = [(0,0)]+transformer_skip+[(0,0)]
        self.single_transformer_skip = [(0,0)]+single_transformer_skip+[(0,0)]
        self._register_func()
    def _register_func(self):
        self.transformer.construct = MethodType(flux_transformer2d_construct, self.transformer)
        self.transformer.transformer_blocks_with_cache = MethodType(transformer_blocks_withcache, self.transformer)
        self.transformer.single_transformer_blocks_with_cache = MethodType(single_transformer_blocks_withcache, self.transformer)
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred, delta_cache, delta_cache_encoder, single_delta_cache = self.transformer(
                ...
                return_dict=False,
                cache_idx=self.transformer_skip[i],
                save_idx=self.transformer_skip[i+1],
                single_cache_idx=self.single_transformer_skip[i],
                single_save_idx=self.single_transformer_skip[i+1],
                delta_cache=delta_cache,
                delta_cache_encoder=delta_cache_encoder,
                single_delta_cache=single_delta_cache,
            )
               
                #TODO: cfg to implement
                # if do_true_cfg:
