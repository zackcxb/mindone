...
class CacheContext:
    ...
    cache_usage_history: List[bool] = dataclasses.field(default_factory=list)
...
def get_cache_usage_history():
    cache_context = get_current_cache_context()
    assert cache_context is not None, "cache_context must be set before"
    return cache_context.cache_usage_history
def update_cache_usage_history(can_use_cache):
    cache_context = get_current_cache_context()
    assert cache_context is not None, "cache_context must be set before"
    cache_context.cache_usage_history.append(can_use_cache)
def clear_cache_usage_history():
    cache_context = get_current_cache_context()
    assert cache_context is not None, "cache_context must be set before"
    cache_context.cache_usage_history.clear()
def get_save_params():
    cache_context = get_current_cache_context()
    assert cache_context is not None, "cache_context must be set before"
    return cache_context.save_params
...
class CachedTransformerBlocks(ms.nn.Cell):
    ...
    def construct(self, hidden_states, encoder_hidden_states, *args, **kwargs):
        ...
        if get_save_params():
            update_cache_usage_history(can_use_cache)
